...
const SHOTS = path.join(ROOT, "screenshots", "v84-minimal");
...
(async () => {
  ...
  for (const vw of [1280, 1440, 1920]) {
    ...
    await pg.goto(`http://127.0.0.1:${PORT}/xueji_student_h5.html?skip-title=1&_=${Date.now()}`, { waitUntil: "load", timeout: 30000 });
    await new Promise(r => setTimeout(r, 1800));
    const out = path.join(SHOTS, `student__${vw}__dashboard.png`);
    await pg.screenshot({ path: out, fullPage: true });
    console.log(`[shot] student dashboard ${vw} -> ${out}`);
    await ctx.close();
  }
  await b.close();
  srv.close();
